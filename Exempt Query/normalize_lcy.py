...
INPUT_FOLDER = BASE_DIR / "Daroda_lc_vrn_files/etc"
MERGE_OUTPUT_FILE = BASE_DIR / "normalized_and_merged_etc_daroda.csv"
...
def _lcy_workbook_to_csv(path: Path, out_path: Path, keywords: List[str]) -> int:
    ...
    if suffix == ".xlsx":
        return stream_xlsx_all_sheets_to_csv(path, out_path, keywords)
    if suffix == ".xls":
...
def phase_convert_workbooks_to_csv_and_delete(folder_path: Path) -> None:
    ...
    min_matches = MIN_HEADER_KEYWORD_MATCHES
    if len(keywords) < min_matches:
        print(
            f"[WARN] csv_converter.HEADER_KEYWORDS must list at least {min_matches} "
            f"strings; have {len(keywords)}. Skipping pre-conversion to CSV.",
            flush=True,
        )
        return
    ...
    for path in targets:
        ...
        try:
            n_rows = _lcy_workbook_to_csv(path, out_csv, keywords)
