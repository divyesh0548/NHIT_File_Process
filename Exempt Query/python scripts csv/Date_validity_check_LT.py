...
#from Date_validity_check import final_df

base_output_path = get_base_output_path()

# Load the Excel files (you need to specify the correct file paths)
#combined_rf_df = final_df
combined_rf_df = pd.read_excel(base_output_path + "/MP Output.xlsx")
working_file_df = pd.read_excel(base_output_path + "/Pass_LT.xlsx")
...
grouped_df = sorted_df.groupby(['Date & Time', 'Veh Reg No.', 'Date Validity Check LT']).size().reset_index(name='Count')

# Merge grouped_df back with combined_rf_df on 'Veh Reg No.' and 'Date & Time' (Left join to mimic Power Query behavior)
final_df = pd.merge(combined_rf_df, grouped_df[['Veh Reg No.', 'Date & Time', 'Date Validity Check LT']], 
...
final_output_path = f"{base_output_path}/LT_output.xlsx"  # Adjust file name as necessary
