import pandas as pd
from config import input_path, output_path, standardize_pass_file_columns

combined_rf_df = pd.read_excel(output_path("combined_with_rf3.xlsx"))
working_file_df = pd.read_excel(input_path("MP_Pass.xlsx"))
...
grouped_df = (
    sorted_df.groupby(["Date & Time", "Veh Reg No.", "Date Validity Check MP"])
    .size()
...
final_df = pd.merge(
    combined_rf_df,
    grouped_df[["Veh Reg No.", "Date & Time", "Date Validity Check MP"]],
...
final_df.to_excel(output_path("MP Output.xlsx"), index=False, sheet_name="Combined with RF 3")
