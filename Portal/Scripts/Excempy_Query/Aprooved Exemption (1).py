...
CONFIG = {
    ...
    "SOURCE_DIR": r"C:\Divyesh\NHIT_File_process\Portal\Scripts\Excempy_Query\Odhaki_Final - Apr May June.xlsx",
    "EXEMPTION_FILE": r"C:\Divyesh\NHIT_File_process\Portal\Scripts\Excempy_Query\Odhaki Cocessionaier\odhaki_concessionaire_vrns.xlsx",
...
def _find_exemption_vrn_column(columns):
    """Accept 'VRN', 'Veh Reg No.', and common aliases (same cleanup as merge_Concessioneir)."""
    aliases = {
        "".join(ch for ch in str(a).strip().casefold() if ch.isalnum())
        for a in (
            "VRN",
            CONFIG["VRN_COLUMN"],
            "Veh Reg No",
    ...
    for col in columns:
        key = "".join(ch for ch in str(col).strip().casefold() if ch.isalnum())
        if key in aliases:
            return col
    return None
...
def apply_exemption_filter(df):
    ...
    try:
        ...
        vrn_col = _find_exemption_vrn_column(exemption_df.columns)
    ...
    if approved_exemption_file and os.path.exists(approved_exemption_file):
        try:
            ...
            if vrn_col:
                ...
                print("  Warning: Could not find VRN / Veh Reg No. column in approved exemption file.")
        except Exception as e:
